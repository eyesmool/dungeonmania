...
public class BattleFacade {
    ...
    public void battle(Game game, Player player, Enemy enemy) {
        ...
        List<Assassin> assassins = game.getMap().getEntities(Assassin.class);
        for (Assassin ass : assassins) {
            if (!ass.isAllied()) continue;
            playerBuff = BattleStatistics.applyBuff(playerBuff, ass.getBattleStatistics());
        }
